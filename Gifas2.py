...
st.set_page_config(
    page_title="Dashboard Überschuss Produzent – KMU",
    page_icon="📊",
    layout="wide",
...
st.markdown('<div class="big-title">Dashboard Produzent – KMU</div>', unsafe_allow_html=True)
...
dE_h["kWh_EVU_KMU"]  = 1000*Kp*(dE_h["kWh_EVU_KMU"])
dE_h["CHF_EVU_KMU"]  = dE_h["kWh_EVU_KMU"] * dE_h["Tarif CHF"]
EVUEKWH              = round(dE_h["kWh_EVU_KMU"].sum(),0)    
EVUECHF              = round(dE_h["CHF_EVU_KMU"].sum(), 0)
st.write("EVUEKWH=",EVUEKWH,"kWh")
st.write("EVUECHF",EVUECHF,"CHF")

# UG1: Überschuss Produzent, Vergütung EVUPCH
...
# Ver_U: Verbleibender Überschuss nach Lieferung an KMU und LEG Kunden
...
def run_analysis(mo: str, topic: str, abbr: str, H1: str, H2: str):
    ...
    st.write("DEBUG df_plot1:", df_plot1.shape, "NaN%:", df_plot1["Valeur"].isna().mean())
    fig1 = px.bar(
        df_plot1,
        x="Heure",
        y="Valeur",
...
def compute_projection(
    val_EVUECHF, val_UG, val_EVUPCH,
    IK, Kp, P, z, r, ES, Sa, SV,
    PK, Un_rate, tevu, tgra, tleg, tbet, pr,
    N_YEARS, VG, val_EVUGECH,
    val_G1, val_G2, val_G3, val_GE, val_RestU,
):
    ...
    #---- Kp
    PW_Kp = [Kp*(1+pr)**jahr for jahr in Jahre]
...
with tab_roh:
    ...
    if fig1 is not None:
        ...
        with colp4:
            ...
            st.metric("Lieferung Gifas an LEG", f"{indic6:,.0f} kWh".replace(",", "’"))
            st.markdown('</div>', unsafe_allow_html=True)

        with colp5:
            st.markdown('<div class="metric-card metric-chf">', unsafe_allow_html=True)
            st.metric("Bruttogewinn KMU", f"{indic4:,.0f} CHF".replace(",", "’"))
            st.markdown('</div>', unsafe_allow_html=True)
...
st.markdown(
    ...
    <div class="footer">
        © 2026 – Tarif TB Rheineck: Jahr 2026 (ohne MWST) - Neukalibrierung der Messwerte des Stromverbrauchs von Gifas per 1. Januar 2024
    </div>
    """,
    unsafe_allow_html=True
)